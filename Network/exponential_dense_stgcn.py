...
class Exp_Dense_STGCN_Block(nn.ModuleDict):
    def __init__(self, in_channels, n_layers, kernel_size, **kwargs):

        super(Exp_Dense_STGCN_Block, self).__init__()
       

        layers = dict()
        prev_in_channels = in_channels // 2
        for i in range(n_layers):
            layer = STGCN_Layer(prev_in_channels * 2, prev_in_channels * 2, kernel_size, 1, **kwargs)
            prev_in_channels = prev_in_channels * 2
...
class StreamSpatialTemporalGraph(nn.Module):
    def __init__(self, in_channels, graph_args, num_class=None, n_layers=6,
        ...
        self.dense_block = Exp_Dense_STGCN_Block(
                                            in_channels=32,
                                            n_layers=n_layers,
...
class Exp_DenseSTGCN_1S(nn.Module):
    def __init__(self, num_class, graph_args, n_layers=6, edge_importance_weighting=True, **kwargs):
        ...
        self.st_gcn = StreamSpatialTemporalGraph(in_channels= 3, 
                                                ...
                                                **kwargs)
        self.fcn = nn.Linear(32 * (2 ** n_layers), num_class)
...
class Exp_DenseSTGCN_2S(nn.Module):
    def __init__(self, num_class, graph_args, n_layers=6, edge_importance_weighting=True,
        ...
        self.fcn = nn.Linear(2 * 32 * (2 ** n_layers), num_class)
